...
public:
     ...
     */
    uint8_t get_crc8(void *ptr, uint16_t length, uint8_t crc8);

    /**
     * @brief calculate crc16 bit given an array of bytes
     * @param ptr pointer to byte array
     * @param length length of data
     * @param crc16 initial value
     * @return calculated crc16 verification byte
     */
    uint16_t get_crc16(void *ptr, uint16_t length, uint16_t crc16);
    
    /**
     * @brief check if the message is corrupted
     * @param ptr pointer to byte array containing the last crc8 byte
     * @param length length of data
     * @return true if successfully appends the crc8 byte
     */
    bool check_crc8(void *ptr, uint16_t length);

    /**
     * @brief check if the message is corrupted
     * @param ptr pointer to byte array containing the last crc16 byte
     * @param length length of data
     * @return true if successfully appends the crc16 byte
     */
    bool check_crc16(void *ptr, uint16_t length);

    /**
     * @brief append the crc8 verification byte to the given data array
     * @param ptr pointer to byte array
     * @param length length of data
     * @return none
     */
    ...
    /**
     * @brief append the crc16 verification byte to the given data array
     * @param ptr pointer to byte array
     * @param length length of data
     * @return none
     */
...
private:
    ...
    data_u      *_rx_body;
    data_u      *_tx_body;
    uint8_t     _rxbuf[MAX_BUFFER_LENGTH];
